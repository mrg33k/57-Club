...
// --- COMPONENT: DROP CARD ---
const DropCard = ({ drop }) => (
  <div className={`group flex flex-col ${COLORS.card} rounded-xl overflow-hidden border ${COLORS.border} shadow-sm transition-all hover:shadow-md`}>
    ...
    <div className="p-5 flex flex-col flex-grow">
      ...
      <div className="pt-4 border-t border-zinc-100 flex items-center justify-between">
         ...
         <Button variant="outline" className="!py-2 !px-4 text-sm" onClick={() => window.open(drop.link, '_blank')}>
            View Access
         </Button>
      </div>
    </div>
...
const Navbar = ({ view, setView, userData, handleLogout, handleJoinClick }) => {
      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm font-medium text-zinc-600">
          {userData?.status === 'active' ? (
            ...
            <>
              <button onClick={() => setView('login')} className="hover:text-zinc-900">Member Login</button>
      ...
      {mobileMenuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-white border-b border-zinc-200 p-6 flex flex-col gap-4 shadow-xl md:hidden">
            {userData?.status === 'active' ? (
                ...
                <>
                    <button onClick={() => {setView('login'); setMobileMenuOpen(false)}} className="text-left font-medium py-2">Log In</button>
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-xl border border-zinc-200 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" required className="w-full p-3 border border-zinc-200 rounded-lg focus:ring-2 focus:ring-zinc-900 outline-none" 
              value={email} onChange={e => setEmail(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">Password</label>
            <input type="password" required className="w-full p-3 border border-zinc-200 rounded-lg focus:ring-2 focus:ring-zinc-900 outline-none" 
...
const CheckoutPage = ({ setView, user, setUserData }) => {
  ...
  // Auto-advance step if user logs in while on this page
  useEffect(() => {
    if (user && step === 'auth') {
      setStep('payment');
    }
  ...
  const handleSignup = async (e) => {
     ...
     try {
        await createUserWithEmailAndPassword(auth, email, password);
        // Step update handled by useEffect above
     } catch (err) {
  ...
  const handleSubscribe = async () => {
     ...
     setTimeout(async () => {
        ...
        try {
           await runTransaction(db, async (transaction) => {
              ...
              if (!sfDoc.exists()) {
                 ...
                 const newCount = sfDoc.data().foundingMembers + 1;
                 newMemberNum = newCount;
                 transaction.update(counterRef, { foundingMembers: newCount });
              }
           });
        } catch (e) {
        ...
        setUserData({ status: 'active', foundingMemberNumber: newMemberNum });
        setStep('success');
     }, 2500);
  };
               ...
               <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm animate-in slide-in-from-left-4">
                  ...
                  <form onSubmit={handleSignup} className="space-y-4">
                     <div>
                        ...
                        <input type="email" required className="w-full p-3 border border-zinc-200 rounded-lg mt-1 outline-none focus:border-zinc-900" 
                          value={email} onChange={e => setEmail(e.target.value)} />
                     </div>
                     <div>
                        <label className="text-xs font-bold uppercase text-zinc-500">Password</label>
                        <input type="password" required className="w-full p-3 border border-zinc-200 rounded-lg mt-1 outline-none focus:border-zinc-900" 
                          value={password} onChange={e => setPassword(e.target.value)} />
                     </div>
                     {authError && <p className="text-red-500 text-sm">{authError}</p>}
                     <Button type="submit" fullWidth variant="primary">Continue to Payment</Button>
                     <p className="text-xs text-center text-zinc-400 mt-2">Already have an account? <button onClick={() => setView('login')} className="underline text-zinc-600">Log in</button></p>
                  </form>
               </div>
            )}
                  ...
                  <div className="bg-zinc-50 p-6 rounded-xl border border-zinc-200 mb-6">
                     ...
                     <div className="space-y-4">
                        <label className="flex gap-3 items-start cursor-pointer">
                           <input type="checkbox" className="mt-1 h-4 w-4 rounded border-zinc-300 text-zinc-900 focus:ring-zinc-900"
                              checked={agreements.taxes} onChange={e => setAgreements({...agreements, taxes: e.target.checked})} />
                        ...
                        <label className="flex gap-3 items-start cursor-pointer">
                           <input type="checkbox" className="mt-1 h-4 w-4 rounded border-zinc-300 text-zinc-900 focus:ring-zinc-900"
                              checked={agreements.availability} onChange={e => setAgreements({...agreements, availability: e.target.checked})} />
                        ...
                        <label className="flex gap-3 items-start cursor-pointer">
                           <input type="checkbox" className="mt-1 h-4 w-4 rounded border-zinc-300 text-zinc-900 focus:ring-zinc-900"
                              checked={agreements.access} onChange={e => setAgreements({...agreements, access: e.target.checked})} />
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (snap.exists()) {
          ...
          if (data.status === 'active' && (view === 'landing' || view === 'checkout' || view === 'login')) {
            setView('portal');
          }
        } else if (view === 'login') {
            // Logged in but no profile (inactive/new) -> send to checkout
            setView('checkout');
        }
      }
      setLoading(false);
    ...
    // Listen to global counter
    const counterRef = doc(db, 'artifacts', appId, 'public', 'counters');
    const unsubCounter = onSnapshot(counterRef, (doc) => {
       if (doc.exists() && doc.data().foundingMembers) {
          setFoundingCount(doc.data().foundingMembers);
       }
    });

    return () => { unsubscribe(); unsubCounter(); };
  }, [view]);
  ...
  useEffect(() => {
    ...
    const unsubscribeDrops = onSnapshot(q, (snapshot) => {
      const loadedDrops = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const seedData = async () => {
     // Seed data omitted for brevity, but logic triggers if drops are empty
     setDrops([
